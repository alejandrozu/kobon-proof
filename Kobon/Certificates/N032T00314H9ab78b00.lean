import Kobon.Geometry

namespace Kobon.Certificates.N032T00314H9ab78b00

set_option maxRecDepth 100000
set_option maxHeartbeats 0

def lines : Array (Line ℤ) := #[
  ⟨87,1073,2257⟩,
  ⟨152,1558,1517⟩,
  ⟨200,1480,-1887⟩,
  ⟨55,160,-308⟩,
  ⟨209,532,812⟩,
  ⟨19,34,-72⟩,
  ⟨287,369,396⟩,
  ⟨1480,1560,-5499⟩,
  ⟨25,16,48⟩,
  ⟨369,205,-1255⟩,
  ⟨660,345,-322⟩,
  ⟨783,377,-3133⟩,
  ⟨73,17,-76⟩,
  ⟨48412,1102,-224315⟩,
  ⟨3944,-160,-2645⟩,
  ⟨2651,-352,-14128⟩,
  ⟨177,-48,-244⟩,
  ⟨1276,-377,-8554⟩,
  ⟨3712,-1189,-8979⟩,
  ⟨91,-39,-904⟩,
  ⟨705,-480,-2144⟩,
  ⟨1763,-1558,-24586⟩,
  ⟨175,-180,-498⟩,
  ⟨32,-37,-134⟩,
  ⟨336,-525,5900⟩,
  ⟨41,-82,-186⟩,
  ⟨608,-1406,7289⟩,
  ⟨14,-37,-96⟩,
  ⟨9,-36,95⟩,
  ⟨32,-200,-335⟩,
  ⟨2,-30,51⟩,
  ⟨40600,358295,-64101412⟩
]

def triangles : List Triple := [
  ⟨0,1,2⟩,
  ⟨0,1,31⟩,
  ⟨0,3,21⟩,
  ⟨0,4,13⟩,
  ⟨0,4,18⟩,
  ⟨0,5,19⟩,
  ⟨0,5,21⟩,
  ⟨0,6,20⟩,
  ⟨0,6,27⟩,
  ⟨0,7,17⟩,
  ⟨0,7,19⟩,
  ⟨0,8,25⟩,
  ⟨0,8,29⟩,
  ⟨0,9,13⟩,
  ⟨0,9,15⟩,
  ⟨0,10,18⟩,
  ⟨0,10,23⟩,
  ⟨0,11,15⟩,
  ⟨0,11,17⟩,
  ⟨0,12,20⟩,
  ⟨0,12,23⟩,
  ⟨0,14,22⟩,
  ⟨0,14,25⟩,
  ⟨0,16,22⟩,
  ⟨0,16,27⟩,
  ⟨0,24,28⟩,
  ⟨0,24,30⟩,
  ⟨0,26,28⟩,
  ⟨0,26,29⟩,
  ⟨1,2,21⟩,
  ⟨1,3,19⟩,
  ⟨1,3,21⟩,
  ⟨1,4,8⟩,
  ⟨1,4,28⟩,
  ⟨1,5,17⟩,
  ⟨1,5,19⟩,
  ⟨1,6,8⟩,
  ⟨1,6,14⟩,
  ⟨1,7,15⟩,
  ⟨1,7,17⟩,
  ⟨1,9,13⟩,
  ⟨1,9,27⟩,
  ⟨1,10,14⟩,
  ⟨1,10,16⟩,
  ⟨1,11,13⟩,
  ⟨1,11,15⟩,
  ⟨1,12,16⟩,
  ⟨1,12,22⟩,
  ⟨1,18,22⟩,
  ⟨1,18,25⟩,
  ⟨1,20,25⟩,
  ⟨1,20,29⟩,
  ⟨1,23,27⟩,
  ⟨1,23,29⟩,
  ⟨1,24,30⟩,
  ⟨1,26,28⟩,
  ⟨1,26,30⟩,
  ⟨2,3,9⟩,
  ⟨2,3,20⟩,
  ⟨2,4,24⟩,
  ⟨2,4,26⟩,
  ⟨2,5,16⟩,
  ⟨2,5,18⟩,
  ⟨2,6,26⟩,
  ⟨2,6,28⟩,
  ⟨2,7,9⟩,
  ⟨2,7,18⟩,
  ⟨2,8,28⟩,
  ⟨2,8,30⟩,
  ⟨2,10,14⟩,
  ⟨2,10,30⟩,
  ⟨2,11,20⟩,
  ⟨2,11,22⟩,
  ⟨2,12,14⟩,
  ⟨2,12,16⟩,
  ⟨2,13,22⟩,
  ⟨2,13,23⟩,
  ⟨2,15,23⟩,
  ⟨2,15,25⟩,
  ⟨2,17,25⟩,
  ⟨2,17,27⟩,
  ⟨2,19,27⟩,
  ⟨2,19,29⟩,
  ⟨2,21,29⟩,
  ⟨3,4,24⟩,
  ⟨3,4,31⟩,
  ⟨3,5,14⟩,
  ⟨3,5,16⟩,
  ⟨3,6,24⟩,
  ⟨3,6,26⟩,
  ⟨3,7,16⟩,
  ⟨3,7,18⟩,
  ⟨3,8,26⟩,
  ⟨3,8,28⟩,
  ⟨3,9,18⟩,
  ⟨3,10,28⟩,
  ⟨3,10,30⟩,
  ⟨3,11,22⟩,
  ⟨3,11,23⟩,
  ⟨3,12,14⟩,
  ⟨3,12,30⟩,
  ⟨3,13,23⟩,
  ⟨3,13,25⟩,
  ⟨3,15,25⟩,
  ⟨3,15,27⟩,
  ⟨3,17,27⟩,
  ⟨3,17,29⟩,
  ⟨3,19,29⟩,
  ⟨3,20,22⟩,
  ⟨4,5,21⟩,
  ⟨4,6,22⟩,
  ⟨4,6,27⟩,
  ⟨4,7,19⟩,
  ⟨4,7,21⟩,
  ⟨4,8,29⟩,
  ⟨4,9,15⟩,
  ⟨4,9,17⟩,
  ⟨4,10,20⟩,
  ⟨4,10,23⟩,
  ⟨4,11,17⟩,
  ⟨4,11,19⟩,
  ⟨4,12,20⟩,
  ⟨4,12,27⟩,
  ⟨4,13,15⟩,
  ⟨4,14,25⟩,
  ⟨4,14,29⟩,
  ⟨4,16,22⟩,
  ⟨4,16,25⟩,
  ⟨4,18,23⟩,
  ⟨4,26,30⟩,
  ⟨4,28,30⟩,
  ⟨5,6,24⟩,
  ⟨5,6,31⟩,
  ⟨5,7,20⟩,
  ⟨5,7,23⟩,
  ⟨5,8,24⟩,
  ⟨5,8,26⟩,
  ⟨5,9,20⟩,
  ⟨5,9,22⟩,
  ⟨5,10,26⟩,
  ⟨5,10,28⟩,
  ⟨5,11,23⟩,
  ⟨5,11,25⟩,
  ⟨5,12,28⟩,
  ⟨5,12,30⟩,
  ⟨5,13,25⟩,
  ⟨5,13,27⟩,
  ⟨5,14,30⟩,
  ⟨5,15,27⟩,
  ⟨5,15,29⟩,
  ⟨5,17,29⟩,
  ⟨5,18,22⟩,
  ⟨6,7,21⟩,
  ⟨6,8,30⟩,
  ⟨6,9,17⟩,
  ⟨6,9,19⟩,
  ⟨6,10,15⟩,
  ⟨6,10,18⟩,
  ⟨6,11,19⟩,
  ⟨6,11,21⟩,
  ⟨6,12,18⟩,
  ⟨6,12,23⟩,
  ⟨6,13,15⟩,
  ⟨6,13,17⟩,
  ⟨6,14,29⟩,
  ⟨6,16,25⟩,
  ⟨6,16,29⟩,
  ⟨6,20,23⟩,
  ⟨6,22,25⟩,
  ⟨6,28,30⟩,
  ⟨7,8,24⟩,
  ⟨7,8,31⟩,
  ⟨7,9,22⟩,
  ⟨7,10,24⟩,
  ⟨7,10,26⟩,
  ⟨7,11,25⟩,
  ⟨7,11,27⟩,
  ⟨7,12,26⟩,
  ⟨7,12,28⟩,
  ⟨7,13,27⟩,
  ⟨7,13,29⟩,
  ⟨7,14,28⟩,
  ⟨7,14,30⟩,
  ⟨7,15,29⟩,
  ⟨7,16,30⟩,
  ⟨7,20,22⟩,
  ⟨7,23,25⟩,
  ⟨8,9,11⟩,
  ⟨8,10,11⟩,
  ⟨8,10,19⟩,
  ⟨8,12,17⟩,
  ⟨8,12,18⟩,
  ⟨8,13,19⟩,
  ⟨8,13,21⟩,
  ⟨8,14,18⟩,
  ⟨8,14,20⟩,
  ⟨8,15,17⟩,
  ⟨8,15,21⟩,
  ⟨8,16,20⟩,
  ⟨8,16,23⟩,
  ⟨8,22,23⟩,
  ⟨8,22,27⟩,
  ⟨8,25,27⟩,
  ⟨9,10,24⟩,
  ⟨9,10,31⟩,
  ⟨9,11,21⟩,
  ⟨9,12,24⟩,
  ⟨9,12,26⟩,
  ⟨9,14,26⟩,
  ⟨9,14,28⟩,
  ⟨9,16,28⟩,
  ⟨9,16,30⟩,
  ⟨9,18,30⟩,
  ⟨9,19,21⟩,
  ⟨9,20,25⟩,
  ⟨9,23,25⟩,
  ⟨9,23,29⟩,
  ⟨9,27,29⟩,
  ⟨10,12,25⟩,
  ⟨10,12,27⟩,
  ⟨10,13,17⟩,
  ⟨10,13,21⟩,
  ⟨10,15,17⟩,
  ⟨10,16,29⟩,
  ⟨10,19,21⟩,
  ⟨10,20,27⟩,
  ⟨10,22,25⟩,
  ⟨10,22,29⟩,
  ⟨11,12,24⟩,
  ⟨11,12,31⟩,
  ⟨11,13,29⟩,
  ⟨11,14,24⟩,
  ⟨11,14,26⟩,
  ⟨11,16,26⟩,
  ⟨11,16,28⟩,
  ⟨11,18,28⟩,
  ⟨11,18,30⟩,
  ⟨11,20,30⟩,
  ⟨11,27,29⟩,
  ⟨12,13,19⟩,
  ⟨12,15,19⟩,
  ⟨12,15,21⟩,
  ⟨12,17,21⟩,
  ⟨12,22,29⟩,
  ⟨12,25,29⟩,
  ⟨13,14,24⟩,
  ⟨13,14,31⟩,
  ⟨13,16,24⟩,
  ⟨13,16,26⟩,
  ⟨13,18,26⟩,
  ⟨13,18,28⟩,
  ⟨13,20,28⟩,
  ⟨13,20,30⟩,
  ⟨13,22,30⟩,
  ⟨14,15,19⟩,
  ⟨14,16,23⟩,
  ⟨14,16,27⟩,
  ⟨14,17,19⟩,
  ⟨14,17,21⟩,
  ⟨14,18,21⟩,
  ⟨14,20,23⟩,
  ⟨14,22,27⟩,
  ⟨15,16,24⟩,
  ⟨15,16,31⟩,
  ⟨15,18,24⟩,
  ⟨15,18,26⟩,
  ⟨15,20,26⟩,
  ⟨15,20,28⟩,
  ⟨15,22,28⟩,
  ⟨15,22,30⟩,
  ⟨15,23,30⟩,
  ⟨16,17,19⟩,
  ⟨16,18,19⟩,
  ⟨16,18,21⟩,
  ⟨16,20,21⟩,
  ⟨17,18,24⟩,
  ⟨17,18,31⟩,
  ⟨17,20,24⟩,
  ⟨17,20,26⟩,
  ⟨17,22,26⟩,
  ⟨17,22,28⟩,
  ⟨17,23,28⟩,
  ⟨17,23,30⟩,
  ⟨17,25,30⟩,
  ⟨18,20,27⟩,
  ⟨18,20,29⟩,
  ⟨18,23,27⟩,
  ⟨18,25,29⟩,
  ⟨19,20,24⟩,
  ⟨19,20,31⟩,
  ⟨19,22,24⟩,
  ⟨19,22,26⟩,
  ⟨19,23,26⟩,
  ⟨19,23,28⟩,
  ⟨19,25,28⟩,
  ⟨19,25,30⟩,
  ⟨19,27,30⟩,
  ⟨21,22,24⟩,
  ⟨21,22,31⟩,
  ⟨21,23,24⟩,
  ⟨21,23,26⟩,
  ⟨21,25,26⟩,
  ⟨21,25,28⟩,
  ⟨21,27,28⟩,
  ⟨21,27,30⟩,
  ⟨21,29,30⟩,
  ⟨23,24,31⟩,
  ⟨24,25,27⟩,
  ⟨24,26,27⟩,
  ⟨24,26,29⟩,
  ⟨24,28,29⟩,
  ⟨25,26,31⟩,
  ⟨27,28,31⟩,
  ⟨29,30,31⟩
]

theorem checked : validate lines triangles 314 = true := by native_decide

theorem lower_bound : LowerBound 32 314 :=
  validate_sound lines triangles 314 checked

#print axioms checked
#print axioms lower_bound

end Kobon.Certificates.N032T00314H9ab78b00
