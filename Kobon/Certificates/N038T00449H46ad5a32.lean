import Kobon.Geometry

namespace Kobon.Certificates.N038T00449H46ad5a32

set_option maxRecDepth 100000
set_option maxHeartbeats 0

def lines : Array (Line ℤ) := #[
  ⟨33,1419,1118⟩,
  ⟨66,1419,-86⟩,
  ⟨185,1702,-1012⟩,
  ⟨49,315,-270⟩,
  ⟨272,1530,-855⟩,
  ⟨56,248,-341⟩,
  ⟨45,138,-92⟩,
  ⟨1298,1034,-5311⟩,
  ⟨98,34,-165⟩,
  ⟨4480,1504,-7755⟩,
  ⟨1316,423,-2151⟩,
  ⟨119,35,-209⟩,
  ⟨1558,418,-2475⟩,
  ⟨1333,301,-2359⟩,
  ⟨3325,608,-5120⟩,
  ⟨10281,1081,-18142⟩,
  ⟨9435,705,-14147⟩,
  ⟨23885,140,-41788⟩,
  ⟨12987,-611,-19082⟩,
  ⟨474,-44,-815⟩,
  ⟨4859,-688,-7088⟩,
  ⟨212,-40,-355⟩,
  ⟨4727,-1015,-6930⟩,
  ⟨7849,-2068,-12760⟩,
  ⟨1034,-286,-1547⟩,
  ⟨592,-176,-935⟩,
  ⟨84,-26,-129⟩,
  ⟨276,-299,-1144⟩,
  ⟨121,-198,-225⟩,
  ⟨160,-296,-407⟩,
  ⟨55,-341,496⟩,
  ⟨31,-279,45⟩,
  ⟨4,-43,26⟩,
  ⟨14,-203,-87⟩,
  ⟨8,-152,19⟩,
  ⟨2,-47,0⟩,
  ⟨25,-1050,504⟩,
  ⟨2130040,32696114,-331900941⟩
]

def triangles : List Triple := [
  ⟨0,1,2⟩,
  ⟨0,1,37⟩,
  ⟨0,3,5⟩,
  ⟨0,4,5⟩,
  ⟨0,4,7⟩,
  ⟨0,6,7⟩,
  ⟨0,6,27⟩,
  ⟨0,8,11⟩,
  ⟨0,8,13⟩,
  ⟨0,9,11⟩,
  ⟨0,9,27⟩,
  ⟨0,10,13⟩,
  ⟨0,10,15⟩,
  ⟨0,12,15⟩,
  ⟨0,12,17⟩,
  ⟨0,14,17⟩,
  ⟨0,14,19⟩,
  ⟨0,16,19⟩,
  ⟨0,16,21⟩,
  ⟨0,18,21⟩,
  ⟨0,18,23⟩,
  ⟨0,20,23⟩,
  ⟨0,20,25⟩,
  ⟨0,22,25⟩,
  ⟨0,22,26⟩,
  ⟨0,24,26⟩,
  ⟨0,24,29⟩,
  ⟨0,28,29⟩,
  ⟨0,28,33⟩,
  ⟨0,30,34⟩,
  ⟨0,30,36⟩,
  ⟨0,31,33⟩,
  ⟨0,31,35⟩,
  ⟨0,32,34⟩,
  ⟨0,32,35⟩,
  ⟨1,2,5⟩,
  ⟨1,3,5⟩,
  ⟨1,3,33⟩,
  ⟨1,4,27⟩,
  ⟨1,4,29⟩,
  ⟨1,6,28⟩,
  ⟨1,6,29⟩,
  ⟨1,7,27⟩,
  ⟨1,7,33⟩,
  ⟨1,8,19⟩,
  ⟨1,8,21⟩,
  ⟨1,9,17⟩,
  ⟨1,9,19⟩,
  ⟨1,10,21⟩,
  ⟨1,10,23⟩,
  ⟨1,11,15⟩,
  ⟨1,11,17⟩,
  ⟨1,12,23⟩,
  ⟨1,12,25⟩,
  ⟨1,13,15⟩,
  ⟨1,13,28⟩,
  ⟨1,14,25⟩,
  ⟨1,14,26⟩,
  ⟨1,16,24⟩,
  ⟨1,16,26⟩,
  ⟨1,18,22⟩,
  ⟨1,18,24⟩,
  ⟨1,20,22⟩,
  ⟨1,20,35⟩,
  ⟨1,30,36⟩,
  ⟨1,31,34⟩,
  ⟨1,31,35⟩,
  ⟨1,32,34⟩,
  ⟨1,32,36⟩,
  ⟨2,3,27⟩,
  ⟨2,3,33⟩,
  ⟨2,4,30⟩,
  ⟨2,4,32⟩,
  ⟨2,5,33⟩,
  ⟨2,6,32⟩,
  ⟨2,6,36⟩,
  ⟨2,7,27⟩,
  ⟨2,7,35⟩,
  ⟨2,8,20⟩,
  ⟨2,8,22⟩,
  ⟨2,9,22⟩,
  ⟨2,9,24⟩,
  ⟨2,10,18⟩,
  ⟨2,10,20⟩,
  ⟨2,11,24⟩,
  ⟨2,11,26⟩,
  ⟨2,12,16⟩,
  ⟨2,12,18⟩,
  ⟨2,13,25⟩,
  ⟨2,13,26⟩,
  ⟨2,14,16⟩,
  ⟨2,14,36⟩,
  ⟨2,15,23⟩,
  ⟨2,15,25⟩,
  ⟨2,17,21⟩,
  ⟨2,17,23⟩,
  ⟨2,19,21⟩,
  ⟨2,19,31⟩,
  ⟨2,28,31⟩,
  ⟨2,28,34⟩,
  ⟨2,29,34⟩,
  ⟨2,29,35⟩,
  ⟨3,4,30⟩,
  ⟨3,4,37⟩,
  ⟨3,6,30⟩,
  ⟨3,6,32⟩,
  ⟨3,7,34⟩,
  ⟨3,7,35⟩,
  ⟨3,8,16⟩,
  ⟨3,8,18⟩,
  ⟨3,9,18⟩,
  ⟨3,9,20⟩,
  ⟨3,10,14⟩,
  ⟨3,10,16⟩,
  ⟨3,11,20⟩,
  ⟨3,11,22⟩,
  ⟨3,12,14⟩,
  ⟨3,12,32⟩,
  ⟨3,13,22⟩,
  ⟨3,13,24⟩,
  ⟨3,15,24⟩,
  ⟨3,15,26⟩,
  ⟨3,17,25⟩,
  ⟨3,17,26⟩,
  ⟨3,19,23⟩,
  ⟨3,19,25⟩,
  ⟨3,21,23⟩,
  ⟨3,21,36⟩,
  ⟨3,27,35⟩,
  ⟨3,28,31⟩,
  ⟨3,28,36⟩,
  ⟨3,29,31⟩,
  ⟨3,29,34⟩,
  ⟨4,6,31⟩,
  ⟨4,6,36⟩,
  ⟨4,7,33⟩,
  ⟨4,8,24⟩,
  ⟨4,8,26⟩,
  ⟨4,9,25⟩,
  ⟨4,9,26⟩,
  ⟨4,10,22⟩,
  ⟨4,10,24⟩,
  ⟨4,11,23⟩,
  ⟨4,11,25⟩,
  ⟨4,12,20⟩,
  ⟨4,12,22⟩,
  ⟨4,13,21⟩,
  ⟨4,13,23⟩,
  ⟨4,14,18⟩,
  ⟨4,14,20⟩,
  ⟨4,15,19⟩,
  ⟨4,15,21⟩,
  ⟨4,16,18⟩,
  ⟨4,16,31⟩,
  ⟨4,17,19⟩,
  ⟨4,17,34⟩,
  ⟨4,27,33⟩,
  ⟨4,28,34⟩,
  ⟨4,28,35⟩,
  ⟨4,29,35⟩,
  ⟨4,32,36⟩,
  ⟨5,6,30⟩,
  ⟨5,6,37⟩,
  ⟨5,7,31⟩,
  ⟨5,7,34⟩,
  ⟨5,8,12⟩,
  ⟨5,8,14⟩,
  ⟨5,9,14⟩,
  ⟨5,9,16⟩,
  ⟨5,10,12⟩,
  ⟨5,10,30⟩,
  ⟨5,11,16⟩,
  ⟨5,11,18⟩,
  ⟨5,13,18⟩,
  ⟨5,13,20⟩,
  ⟨5,15,20⟩,
  ⟨5,15,22⟩,
  ⟨5,17,22⟩,
  ⟨5,17,24⟩,
  ⟨5,19,24⟩,
  ⟨5,19,26⟩,
  ⟨5,21,25⟩,
  ⟨5,21,26⟩,
  ⟨5,23,25⟩,
  ⟨5,23,32⟩,
  ⟨5,27,34⟩,
  ⟨5,27,35⟩,
  ⟨5,28,32⟩,
  ⟨5,28,36⟩,
  ⟨5,29,31⟩,
  ⟨5,29,36⟩,
  ⟨5,33,35⟩,
  ⟨6,8,23⟩,
  ⟨6,8,25⟩,
  ⟨6,9,21⟩,
  ⟨6,9,23⟩,
  ⟨6,10,25⟩,
  ⟨6,10,26⟩,
  ⟨6,11,19⟩,
  ⟨6,11,21⟩,
  ⟨6,12,24⟩,
  ⟨6,12,26⟩,
  ⟨6,13,17⟩,
  ⟨6,13,19⟩,
  ⟨6,14,22⟩,
  ⟨6,14,24⟩,
  ⟨6,15,17⟩,
  ⟨6,15,35⟩,
  ⟨6,16,20⟩,
  ⟨6,16,22⟩,
  ⟨6,18,20⟩,
  ⟨6,18,34⟩,
  ⟨6,27,33⟩,
  ⟨6,28,35⟩,
  ⟨6,29,33⟩,
  ⟨6,31,34⟩,
  ⟨7,8,10⟩,
  ⟨7,8,37⟩,
  ⟨7,9,10⟩,
  ⟨7,9,12⟩,
  ⟨7,11,12⟩,
  ⟨7,11,14⟩,
  ⟨7,13,14⟩,
  ⟨7,13,16⟩,
  ⟨7,15,16⟩,
  ⟨7,15,18⟩,
  ⟨7,17,18⟩,
  ⟨7,17,20⟩,
  ⟨7,19,20⟩,
  ⟨7,19,22⟩,
  ⟨7,21,22⟩,
  ⟨7,21,24⟩,
  ⟨7,23,24⟩,
  ⟨7,23,26⟩,
  ⟨7,25,26⟩,
  ⟨7,25,30⟩,
  ⟨7,28,30⟩,
  ⟨7,28,32⟩,
  ⟨7,29,32⟩,
  ⟨7,29,36⟩,
  ⟨7,31,36⟩,
  ⟨8,9,27⟩,
  ⟨8,10,30⟩,
  ⟨8,11,27⟩,
  ⟨8,12,30⟩,
  ⟨8,13,29⟩,
  ⟨8,14,32⟩,
  ⟨8,15,29⟩,
  ⟨8,15,33⟩,
  ⟨8,16,32⟩,
  ⟨8,17,28⟩,
  ⟨8,17,33⟩,
  ⟨8,18,36⟩,
  ⟨8,19,28⟩,
  ⟨8,20,36⟩,
  ⟨8,21,35⟩,
  ⟨8,22,31⟩,
  ⟨8,23,35⟩,
  ⟨8,24,31⟩,
  ⟨8,25,34⟩,
  ⟨8,26,34⟩,
  ⟨9,10,37⟩,
  ⟨9,11,29⟩,
  ⟨9,12,30⟩,
  ⟨9,13,29⟩,
  ⟨9,13,33⟩,
  ⟨9,14,30⟩,
  ⟨9,15,28⟩,
  ⟨9,15,33⟩,
  ⟨9,16,32⟩,
  ⟨9,17,28⟩,
  ⟨9,18,32⟩,
  ⟨9,19,35⟩,
  ⟨9,20,36⟩,
  ⟨9,21,35⟩,
  ⟨9,22,36⟩,
  ⟨9,23,34⟩,
  ⟨9,24,31⟩,
  ⟨9,25,34⟩,
  ⟨9,26,31⟩,
  ⟨10,11,27⟩,
  ⟨10,12,32⟩,
  ⟨10,13,27⟩,
  ⟨10,14,32⟩,
  ⟨10,15,29⟩,
  ⟨10,16,36⟩,
  ⟨10,17,29⟩,
  ⟨10,17,33⟩,
  ⟨10,18,36⟩,
  ⟨10,19,28⟩,
  ⟨10,19,33⟩,
  ⟨10,20,31⟩,
  ⟨10,21,28⟩,
  ⟨10,22,31⟩,
  ⟨10,23,35⟩,
  ⟨10,24,34⟩,
  ⟨10,25,35⟩,
  ⟨10,26,34⟩,
  ⟨11,12,37⟩,
  ⟨11,13,28⟩,
  ⟨11,13,33⟩,
  ⟨11,14,30⟩,
  ⟨11,15,28⟩,
  ⟨11,16,30⟩,
  ⟨11,17,35⟩,
  ⟨11,18,32⟩,
  ⟨11,19,35⟩,
  ⟨11,20,32⟩,
  ⟨11,21,34⟩,
  ⟨11,22,36⟩,
  ⟨11,23,34⟩,
  ⟨11,24,36⟩,
  ⟨11,25,31⟩,
  ⟨11,26,31⟩,
  ⟨11,29,33⟩,
  ⟨12,13,27⟩,
  ⟨12,14,36⟩,
  ⟨12,15,27⟩,
  ⟨12,16,36⟩,
  ⟨12,17,29⟩,
  ⟨12,18,31⟩,
  ⟨12,19,29⟩,
  ⟨12,19,33⟩,
  ⟨12,20,31⟩,
  ⟨12,21,28⟩,
  ⟨12,21,33⟩,
  ⟨12,22,34⟩,
  ⟨12,23,28⟩,
  ⟨12,24,34⟩,
  ⟨12,25,35⟩,
  ⟨12,26,35⟩,
  ⟨13,14,37⟩,
  ⟨13,15,35⟩,
  ⟨13,16,30⟩,
  ⟨13,17,35⟩,
  ⟨13,18,30⟩,
  ⟨13,19,34⟩,
  ⟨13,20,32⟩,
  ⟨13,21,34⟩,
  ⟨13,22,32⟩,
  ⟨13,23,31⟩,
  ⟨13,24,36⟩,
  ⟨13,25,31⟩,
  ⟨13,26,36⟩,
  ⟨14,15,27⟩,
  ⟨14,16,31⟩,
  ⟨14,17,27⟩,
  ⟨14,18,31⟩,
  ⟨14,19,29⟩,
  ⟨14,20,34⟩,
  ⟨14,21,29⟩,
  ⟨14,21,33⟩,
  ⟨14,22,34⟩,
  ⟨14,23,28⟩,
  ⟨14,23,33⟩,
  ⟨14,24,35⟩,
  ⟨14,25,28⟩,
  ⟨14,26,35⟩,
  ⟨15,16,37⟩,
  ⟨15,17,34⟩,
  ⟨15,18,30⟩,
  ⟨15,19,34⟩,
  ⟨15,20,30⟩,
  ⟨15,21,31⟩,
  ⟨15,22,32⟩,
  ⟨15,23,31⟩,
  ⟨15,24,32⟩,
  ⟨15,25,36⟩,
  ⟨15,26,36⟩,
  ⟨16,17,27⟩,
  ⟨16,18,34⟩,
  ⟨16,19,27⟩,
  ⟨16,20,34⟩,
  ⟨16,21,29⟩,
  ⟨16,22,35⟩,
  ⟨16,23,29⟩,
  ⟨16,23,33⟩,
  ⟨16,24,35⟩,
  ⟨16,25,28⟩,
  ⟨16,25,33⟩,
  ⟨16,26,28⟩,
  ⟨17,18,37⟩,
  ⟨17,19,31⟩,
  ⟨17,20,30⟩,
  ⟨17,21,31⟩,
  ⟨17,22,30⟩,
  ⟨17,23,36⟩,
  ⟨17,24,32⟩,
  ⟨17,25,36⟩,
  ⟨17,26,32⟩,
  ⟨18,19,27⟩,
  ⟨18,20,35⟩,
  ⟨18,21,27⟩,
  ⟨18,22,35⟩,
  ⟨18,23,29⟩,
  ⟨18,24,28⟩,
  ⟨18,25,29⟩,
  ⟨18,25,33⟩,
  ⟨18,26,28⟩,
  ⟨18,26,33⟩,
  ⟨19,20,37⟩,
  ⟨19,21,36⟩,
  ⟨19,22,30⟩,
  ⟨19,23,36⟩,
  ⟨19,24,30⟩,
  ⟨19,25,32⟩,
  ⟨19,26,32⟩,
  ⟨20,21,27⟩,
  ⟨20,22,28⟩,
  ⟨20,23,27⟩,
  ⟨20,24,28⟩,
  ⟨20,24,33⟩,
  ⟨20,25,29⟩,
  ⟨20,26,29⟩,
  ⟨20,26,33⟩,
  ⟨21,22,37⟩,
  ⟨21,23,32⟩,
  ⟨21,24,30⟩,
  ⟨21,25,32⟩,
  ⟨21,26,30⟩,
  ⟨22,23,27⟩,
  ⟨22,24,29⟩,
  ⟨22,24,33⟩,
  ⟨22,25,27⟩,
  ⟨22,26,29⟩,
  ⟨22,28,33⟩,
  ⟨23,24,37⟩,
  ⟨23,25,30⟩,
  ⟨23,26,30⟩,
  ⟨24,25,27⟩,
  ⟨24,26,27⟩,
  ⟨25,26,37⟩,
  ⟨27,28,30⟩,
  ⟨27,28,37⟩,
  ⟨27,29,30⟩,
  ⟨27,29,32⟩,
  ⟨27,31,32⟩,
  ⟨27,31,36⟩,
  ⟨27,34,36⟩,
  ⟨29,30,37⟩,
  ⟨30,31,33⟩,
  ⟨30,32,33⟩,
  ⟨30,32,35⟩,
  ⟨30,34,35⟩,
  ⟨31,32,37⟩,
  ⟨33,34,36⟩,
  ⟨33,34,37⟩,
  ⟨33,35,36⟩,
  ⟨35,36,37⟩
]

theorem checked : validate lines triangles 449 = true := by native_decide

theorem lower_bound : LowerBound 38 449 :=
  validate_sound lines triangles 449 checked

#print axioms checked
#print axioms lower_bound

end Kobon.Certificates.N038T00449H46ad5a32
