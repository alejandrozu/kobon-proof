...
/-! The classical Furedi--Palasti construction. This is prior mathematics,
formalized here to supply the all-order baseline for the verified envelope. -/
namespace Kobon.FurediPalasti

noncomputable def line (t : ℝ) : Line ℝ :=
  ⟨Real.sin t, Real.cos t, Real.sin (3*t)⟩

theorem det_line (x y : ℝ) : det (line x) (line y) = Real.sin (x-y) := by
  simp [det,line,Real.sin_sub]

theorem eval_line (x y z : ℝ) :
    evalVertex (line z) (line x) (line y) =
      4*Real.sin (x-y)*Real.sin (z-y)*Real.sin (z-x)*Real.sin (x+y+z) := by
  simp only [evalVertex,vertex,det,line,Real.sin_add,Real.cos_add,Real.sin_sub,
    Real.sin_three_mul]
  have hx := Real.sin_sq_add_cos_sq x
  have hy := Real.sin_sq_add_cos_sq y
  have hz := Real.sin_sq_add_cos_sq z
  linear_combination
    (4*(Real.cos y*Real.sin z-Real.cos z*Real.sin y)*
      (Real.cos x*Real.cos y*Real.sin y*Real.sin z^2
       +Real.cos x*Real.cos z*Real.sin y^2*Real.sin z
       -Real.cos y^2*Real.sin x*Real.sin z^2
       -Real.cos y*Real.cos z*Real.sin x*Real.sin y*Real.sin z
       -Real.cos z^2*Real.sin x*Real.sin y^2
       -Real.sin x*Real.sin y^2*Real.sin z^2))*hx
    +(4*(Real.cos x*Real.sin y*Real.sin z^3
       +Real.cos y*Real.cos z^2*Real.sin x^3*Real.sin z
       +Real.cos y*Real.sin x^3*Real.sin z^3
       -Real.cos y*Real.sin x*Real.sin z^3
       -Real.cos z^3*Real.sin x^3*Real.sin y
       -Real.cos z*Real.sin x^3*Real.sin y*Real.sin z^2))*hy
    +(-4*(Real.cos x*Real.sin y^3*Real.sin z
       -Real.cos y*Real.sin x^3*Real.sin z
       +Real.cos z*Real.sin x^3*Real.sin y
       -Real.cos z*Real.sin x*Real.sin y^3))*hz

noncomputable def angle (n i : ℕ) : ℝ := ((i:ℝ)+1/2)/(n:ℝ)*Real.pi
...
noncomputable def sumAngle (n s : ℕ) : ℝ := ((s:ℝ)+3/2)/(n:ℝ)*Real.pi
...
theorem sumAngle_band {n s b : ℕ} (hn : 0 < n)
    (hlo : b*n ≤ s+1) (hhi : s+2 ≤ (b+1)*n) :
    (b:ℝ)*Real.pi < sumAngle n s ∧ sumAngle n s < ((b:ℝ)+1)*Real.pi := by
  have hnR : (0:ℝ)<n := by exact_mod_cast hn
  have hloR : (b:ℝ)*n ≤ s+1 := by exact_mod_cast hlo
  have hhiR : (s:ℝ)+2 ≤ (b+1)*n := by exact_mod_cast hhi
  have hl : (b:ℝ) < ((s:ℝ)+3/2)/n := (lt_div_iff₀ hnR).mpr (by linarith)
  have hh : ((s:ℝ)+3/2)/n < (b:ℝ)+1 := (div_lt_iff₀ hnR).mpr (by linarith)
  exact ⟨mul_lt_mul_of_pos_right hl Real.pi_pos,mul_lt_mul_of_pos_right hh Real.pi_pos⟩

theorem sum_sin_pos0 {n s : ℕ} (hn : 0 < n) (hs : s+2≤n) :
...
theorem sum_sin_neg1 {n s : ℕ} (hn : 0 < n) (hl : n≤s+1) (hh : s+2≤2*n) :
...
theorem sum_sin_pos2 {n s : ℕ} (hn : 0 < n) (hl : 2*n≤s+1) (hh : s+2≤3*n) :
    0 < Real.sin (sumAngle n s) := by
...
theorem oriented_line (x y z : ℝ) :
    orientedEval (line z) (line x) (line y) =
      (4*Real.sin (x-y)^2)*
        (Real.sin (z-y)*Real.sin (z-x)*Real.sin (x+y+z)) := by
  rw [orientedEval,eval_line,det_line]
  ring

def LowBand (n s : ℕ) : Prop := s+2=n ∨ s+1=n
def HighBand (n s : ℕ) : Prop := s+2=2*n ∨ s+1=2*n
...
def lower (n : ℕ) : ℕ := (n*(n-3)+2)/3

theorem lower_bound (n : ℕ) (hn : 3≤n) : LowerBound n (lower n) := by
  classical
  ...
  have hi : Function.Injective convert := by
    ...
    simp [e1,e2,e3]
  refine ⟨arrangement n,noParallel n,os.toList.map convert,?_,?_,?_⟩
